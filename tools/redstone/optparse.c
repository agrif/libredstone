/*
 * This file is part of Rick Litherland's optparse.c, a C-flavored
 * version of Python's optparse module.
 *
 * Documentation: http://orangeclawhammer.net/optparse/optparse.pdf
 * Source:        http://orangeclawhammer.net/optparse/optparse-c.tar.gz
 */
...
int opt_store_choice_abbr(char *arg, void *data)
{
    ...
    if (choices[j]) {
        ...
        fprintf(stderr, "ambiguous choice '%s' for option %s\n%*s(%s",
                arg, opt_name(), (int)(strlen(globals.prog)+2), "", choices[i]);
        for (;;) {
            i = j;
            for (j = i + 1;
            ...
            if (o) {
                ...
                if (o2) {
                    fprintf(stderr, "%s: ambiguous option %s\n%*s(%s",
                            globals.prog, a, (int)(strlen(globals.prog)+2),
                            "", o->lf);
                    for (;;) {
                        o = o2;
                        o2 = findlf(o + 1, a);
